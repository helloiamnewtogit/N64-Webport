...
var AUDIO_RING_BUFFER_SIZE = 64000;
var ASSET_CACHE_NAME = 'ultimate-n64onlineemu-assets-v1';
...
class MyClass {
    ...
    async writeAssets(){
        let file = 'assets.zip';
        let responseText = await this.getCachedOrFetchBinary(file);
    ...
    async getCachedOrFetchBinary(url) {
        let cachedBytes = await this.getCachedBinary(url);
        if (cachedBytes) {
            return cachedBytes;
        }

        let downloadedBytes = await this.downloadFile(url);
        this.cacheBinary(url, downloadedBytes).catch((error) => {
            console.warn('Unable to cache asset', url, error);
        });

        return downloadedBytes;
    }

    async getCachedBinary(url) {
        if (!('caches' in window)) {
            return null;
        }
    ...
    async cacheBinary(url, byteArray) {
        if (!('caches' in window)) {
            return;
        }

        let cache = await caches.open(ASSET_CACHE_NAME);
        await cache.put(url, new Response(byteArray, {
            headers: {
    ...
    async initAudio() {
        ...
        if (!this.audioInited)
        {
            ...
            this.audioBufferResampled = new Int16Array(Module.HEAP16.buffer,Module._neilGetSoundBufferResampledAddress(),AUDIO_RING_BUFFER_SIZE);
    ...
    getAudioBufferedSamples(writePosition = this.audioWritePosition, readPosition = this.audioReadPosition) {
        if (writePosition >= readPosition) {
            return writePosition - readPosition;
        }

        return AUDIO_RING_BUFFER_SIZE - readPosition + writePosition;
    }

    hasEnoughSamples(){
        return this.getAudioBufferedSamples() >= AUDIOBUFFSIZE * 2;
    }
    ...
    AudioProcessRecurring(audioProcessingEvent){
        ...
        for (let sample = 0; sample < AUDIOBUFFSIZE; sample++) {
            ...
            if (this.audioWritePosition != this.audioReadPosition) {
                ...
                if (this.audioReadPosition == AUDIO_RING_BUFFER_SIZE) {
                    this.audioReadPosition = 0;
                }
            }
            else {
        ...
        this.setRemainingAudio(this.getAudioBufferedSamples());
